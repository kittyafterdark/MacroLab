...
import { MACROLAB_BEAKER_ICON, MACROLAB_COAT_ICON, MACROLAB_HOT_PLATE_ICON, MACROLAB_PIPETTE_ICON, } from './icons.js';
const PIPETTE_SURFACES = [
...
function iconMarkup(icon, className = 'ml-icon') {
...
function targetLabel(target) {
    ...
    if (explicit?.trim())
        return explicit.trim().slice(0, 90);
...
function insertIntoEditable(target, text) {
    target.focus();
    if (target instanceof HTMLTextAreaElement || target instanceof HTMLInputElement) {
...
function visibleEditableCandidates() {
    return Array.from(document.querySelectorAll('textarea, input, [contenteditable="true"], [contenteditable=""]'))
        .filter(isEditable)
        .filter((node) => {
...
export function setup(ctx) {
    ...
    let hotPlateModal = null;
    let pipetteModal = null;
  ...
  `);
    ...
    const send = (payload, statusText) => {
        ...
        if (statusText)
            setDrawerStatus(statusText, 'idle');
        return id;
    };
    ...
    const setDrawerStatus = (text, kind) => {
    ...
    const instanceAction = (macroNameValue, instance, action) => {
    ...
    const renderDecisionGroups = (container, state, compact) => {
        ...
        for (const [, items] of groupedDecisions(state)) {
            ...
            for (const item of items) {
                ...
                if (decision.locked)
                    labelLine.append(el('span', 'ml-pill', 'locked'));
    ...
    const renderVariables = (state) => {
        ...
        for (const scope of ['local', 'chat', 'global']) {
            ...
            if (!entries.length)
                section.append(el('div', 'ml-meta', `No ${scope} variables.`));
    ...
    const launcherNodes = [];
    const mountLauncher = (point, label, onClick, withBadge = false) => {
        try {
    ...
    // Pipette ---------------------------------------------------------------
    const renderPipette = (surfaceLabel) => {
        if (!pipetteModal)
            return;
        const root = pipetteModal.root;
        root.replaceChildren();
        const body = el('section', 'ml-root ml-modal');
        const header = el('div', 'ml-surface-header');
        ...
        for (const ref of refs) {
            ...
            if (existing)
                ...
                counts.set(lower, { name: ref.name, count: 1, definition, kind });
        }
        const targetCard = el('section', 'ml-card');
        targetCard.append(el('h2', 'ml-label', 'Current field'), el('div', 'ml-title', targetLabel(target)), el('div', 'ml-meta', `${text.length.toLocaleString()} characters · ${refs.length} macro reference${refs.length === 1 ? '' : 's'}`));
        const foundCard = el('section', 'ml-card');
        foundCard.append(el('h2', 'ml-label', 'Detected here'));
        if (!counts.size)
            foundCard.append(el('div', 'ml-empty', 'No macro references in this field yet. Use the registered list below to insert one.'));
        for (const item of [...counts.values()].sort((a, b) => a.name.localeCompare(b.name))) {
            ...
            if (item.definition)
                title.append(el('span', 'ml-pill', `${item.definition.decisions.length} sticky`));
        ...
        if (!macros.length)
            insertCard.append(el('div', 'ml-meta', 'No registered macros yet. Open MacroLab to create one.'));
    ...
    const openPipette = (surfaceLabel) => {
        hotPlateModal?.dismiss?.();
        if (pipetteModal)
            pipetteModal.dismiss();
    ...
    for (const surface of PIPETTE_SURFACES)
        mountLauncher(surface.point, `Open MacroLab Pipette for ${surface.label}`, () => openPipette(surface.label));
    // Backend messages ------------------------------------------------------
    ...
    const unsubscribeBackend = ctx.onBackendMessage((payload) => {
        ...
        if (payload.type === 'macrolab:state') {
            ...
            applyState(state);
            if (!macroForm.hidden && state.notice?.startsWith('Saved '))
                closeMacroForm();
            return;
        }
