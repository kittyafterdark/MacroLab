...
type BackendPayload = ResolveResult | StateResult | BackendError | Record<string, unknown>
...
export function setup(ctx: any): Cleanup {
  ...
  // Resolution panel -------------------------------------------------------
  const resolutionIntro = el(
    'p',
    'lml-intro',
    'Dry-run the real Lumiverse macro engine. Existing sticky decisions are honored, but preview-only rolls and state mutations are not committed.',
  )
  ...
  previewStatePanel.append(
    previewSummary,
    el('p', 'lml-section-note', 'This snapshot is loaded before the dry run. Macro Lab internal decision variables are hidden here and shown in Macros & State instead.'),
    previewGrid,
  )
  ...
  const stateIntro = el(
    'p',
    'lml-intro',
    'Register a macro name around ordinary Lumi syntax. Inside that registered body, native {{pick}} and {{random}} nodes become sticky per chat + instance and can be rerolled independently.',
  )
  ...
  decisionsCard.append(
    decisionsHeading,
    el('p', 'lml-section-note', '↻ rerolls only that node. Locked nodes survive bulk rerolls/resets. Right-click or long-press a decision for the same actions.'),
    decisionList,
  )
  ...
  variablesCard.append(
    variablesHeading,
    el('p', 'lml-section-note', 'These are real Lumi variables, not a shadow store. Edit/delete here and normal macros see the change immediately.'),
    variablesList,
  )
  ...
  let pendingResolveId: string | null = null
  const pendingStateIds = new Set<string>()
  let resolvedText = ''
  ...
  const setResolveStatus = (message: string, state: 'idle' | 'working' | 'success' | 'error') => {
    resolveStatus.textContent = message
    resolveStatus.dataset.state = state
  }
  const setStateStatus = (message: string, state: 'idle' | 'working' | 'success' | 'error') => {
    stateStatus.textContent = message
    stateStatus.dataset.state = state
  }
  const setResolveBusy = (busy: boolean) => {
  ...
  const sendStateRequest = (payload: Record<string, unknown>, statusMessage?: string): string => {
    const requestId = createRequestId()
    pendingStateIds.add(requestId)
    if (statusMessage) setStateStatus(statusMessage, 'working')
  ...
  const decisionAction = (key: string, action: 'reroll' | 'toggle_lock' | 'reset') => {
    hideContextMenu()
  ...
  const showDecisionMenu = (x: number, y: number, key: string, state: DecisionState) => {
    contextMenu.replaceChildren()
  ...
  const attachDecisionGestures = (row: HTMLElement, key: string, state: DecisionState) => {
    ...
    const cancelLongPress = (event?: PointerEvent) => {
      if (event && longPressStart) {
        ...
        if (distance < 8 && event.type === 'pointermove') return
      }
      if (longPressTimer) clearTimeout(longPressTimer)
      longPressTimer = null
      longPressStart = null
  ...
  const renderPreviewVariables = (snapshot: VariableSnapshot | null) => {
    previewGrid.replaceChildren()
    if (!snapshot) {
      previewStatePanel.hidden = true
      return
    }
    const sections: Array<[VariableScope, Record<string, string>]> = [
      ['chat', snapshot.chat],
    ...
    if (!count) previewGrid.append(el('span', 'lml-state-value', 'No native variables found.'))
  ...
  const openMacroForm = (definition?: MacroDefinition) => {
  ...
  const deleteMacro = (definition: MacroDefinition) => {
    if (!window.confirm(`Delete {{${definition.name}}}? Its sticky state in the active chat will also be removed.`)) return
    sendStateRequest({ type: 'lumi_macro_lab:delete_macro', name: definition.name }, `Deleting {{${definition.name}}}…`)
  }

  const renderMacros = (macros: MacroDefinition[]) => {
  ...
  const renderDecisions = (decisions: StateResult['decisions']) => {
    ...
    const groups = new Map<string, StateResult['decisions']>()
    ...
    for (const items of groups.values()) {
      ...
      headingText.append(
        el('h3', 'lml-title lml-code', `{{${first.macroName}${first.instance === 'default' ? '' : `::${first.instance}`}}}`),
        el('span', 'lml-pill', `${items.length} node${items.length === 1 ? '' : 's'}`),
      )
      ...
      rerollAll.addEventListener('click', () => {
        sendStateRequest(
          { type: 'lumi_macro_lab:instance_action', macroName: first.macroName, instance: first.instance, action: 'reroll' },
          `Rerolling ${first.macroName}/${first.instance}…`,
        )
      })
      resetAll.addEventListener('click', () => {
        sendStateRequest(
          { type: 'lumi_macro_lab:instance_action', macroName: first.macroName, instance: first.instance, action: 'reset' },
          `Resetting ${first.macroName}/${first.instance}…`,
        )
      })
      ...
      for (const item of items) {
        ...
        label.append(
          el('span', 'lml-title lml-code', state.decisionId),
          el('span', 'lml-pill', state.kind),
          ...(state.locked ? [el('span', 'lml-pill', 'locked')] : []),
        )
  ...
  const renderVariables = (snapshot: VariableSnapshot) => {
    variablesList.replaceChildren()
    const scopes: VariableScope[] = ['local', 'chat', 'global']
      ...
      if (!entries.length) {
        ...
        for (const [key, value] of entries) {
          ...
          remove.addEventListener('click', () => {
            if (!window.confirm(`Delete ${scopePrefix(scope)}${key}?`)) return
  ...
  const renderState = (state: StateResult) => {
    ...
    if (state.notice) setStateStatus(state.notice, 'success')
    else setStateStatus('State refreshed. The dice are contained. Mostly.', 'success')
  ...
  const switchPanel = (panel: 'resolution' | 'macros') => {
  ...
  const onCopyClick = async () => {
    ...
    try {
      if (navigator.clipboard?.writeText) await navigator.clipboard.writeText(resolvedText)
      else if (!copyTextFallback(resolvedText)) throw new Error('Clipboard copy was rejected')
  ...
  const unsubscribeBackend = ctx.onBackendMessage((payload: BackendPayload) => {
    ...
    if (payload.type === 'lumi_macro_lab:result') {
      ...
      if (result.requestId !== pendingResolveId) return
      pendingResolveId = null
      setResolveBusy(false)
    ...
    if (payload.type === 'lumi_macro_lab:state') {
      const result = payload as StateResult
      if (!pendingStateIds.has(result.requestId)) return
