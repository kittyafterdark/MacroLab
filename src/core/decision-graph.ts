export type DecisionKind = 'pick' | 'random'
...
function collapseBalancedMacros(input: string): string {
  let text = input
...
function normalizedContext(input: string, takeFromEnd: boolean): string {
...
function replaceFriendlyMacros(input: string): string {
  let text = input
...
function labelFromLeft(body: string, offset: number, kind: DecisionKind, ordinal: number): string {
  ...
  if (label.length > 64) label = label.slice(-64).replace(/^\S+\s*/, '')
  if (label.length < 2) label = kind === 'pick' ? `Choice ${ordinal + 1}` : `Random value ${ordinal + 1}`
  return label
}

export function findMacroEnd(body: string, start: number): number {
  if (body.slice(start, start + 2) !== '{{') return -1
  let depth = 0
...
function sourcePreview(label: string, kind: DecisionKind): string {
...
function stochasticOccurrences(body: string): RawOccurrence[] {
  const matches: RawOccurrence[] = []
...
export function scanDecisionDescriptors(body: string): DecisionDescriptor[] {
...
export function instrumentMacroBody(
  body: string,
  macroName: string,
  instance: string,
  internalNames: { pick: string; random: string },
): string {
  const occurrences = stochasticOccurrences(body)
  const descriptors = scanDecisionDescriptors(body)
  if (!occurrences.length) return body
...
export function scanMacroReferences(text: string): MacroReference[] {
...
export function definitionFingerprint(body: string): string {
