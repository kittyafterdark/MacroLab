import { fnv1a, type DecisionKind } from './decision-graph.js'
...
export function decisionKey(macroName: string, instance: string, decisionId: string): string {
  return `${STATE_PREFIX}${fnv1a(`${macroName}\u0000${instance}`)}_${decisionId}`
}

export function serializeDecision(state: DecisionState): string {
...
export function parseDecision(value: unknown): DecisionState | null {
  ...
  try {
    ...
    const history = Array.isArray(parsed.history)
      ? parsed.history
          .filter((entry): entry is DecisionHistoryEntry => Boolean(entry) && typeof entry === 'object' && typeof (entry as DecisionHistoryEntry).value === 'string')
          .map((entry) => ({
...
export function pushHistory(state: DecisionState): DecisionHistoryEntry[] {
